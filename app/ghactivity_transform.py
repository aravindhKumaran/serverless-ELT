...
def transform_to_parquet(file_name, bucket_name, target_folder):
    ...
    df_reader = pd.read_json(f's3://{bucket_name}/landing/ghactivity/{file_name}',
        lines=True, 
        orient='records',
        chunksize=10000
    )
    ...
    return {
        ...
        'last_run_target_file_pattern': f's3://{bucket_name}/{target_folder}/year={year}/month={month}/dayofmonth={dayofmonth}/part-{year}-{month}-{dayofmonth}-{hour}',
        'status_code': 200
    }
